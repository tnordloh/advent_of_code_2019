...
def get_input(phase, input)
  x = File.readlines('input.txt').first.split(',').map(&:to_i)
  op_coder = OpCoder.new(x, [phase, input])
  op_coder.process_all
  op_coder.output
end
possibles = [0, 1, 2, 3, 4].permutation(5).each_with_object({}) do |permutation, acc|
  amp_a = get_input(permutation[0], 0)
  amp_b = get_input(permutation[1], amp_a)
  amp_c = get_input(permutation[2], amp_b)
  amp_d = get_input(permutation[3], amp_c)
  amp_e = get_input(permutation[4], amp_d)
  acc[amp_e] = possibles
end
